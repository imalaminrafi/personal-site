...
export default function AdminAbout() {
  ...
  return (
    <AdminLayout title="About Page">
      ...
      {/* Editing Panel */}
      {editingSection && (
        <div className="bg-white dark:bg-[#162032] rounded-2xl border border-zinc-200 dark:border-white/[0.06] p-5 sm:p-6">
...
function Input({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
      className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-zinc-50 dark:bg-[#162032]/50 text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
  );
}
...
function Textarea({ value, onChange, rows = 3, placeholder }: { value: string; onChange: (v: string) => void; rows?: number; placeholder?: string }) {
  return (
    <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={rows} placeholder={placeholder}
      className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-zinc-50 dark:bg-[#162032]/50 text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-500/40 resize-none" />
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="Focus Items">
        {items.map((item, i) => (
          <div key={i} className="p-4 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-zinc-50 dark:bg-[#162032]/40 mb-3">
  ...
  return (
    <Field label="Tools">
      <div className="flex flex-wrap gap-2 mb-3">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-zinc-100 dark:bg-[#1E293B] border border-zinc-200 dark:border-white/[0.06]">
