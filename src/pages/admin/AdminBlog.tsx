      ...
      <AdminLayout title={editId ? "Edit Post" : "New Post"}>
        <div className="max-w-4xl mx-auto space-y-8 pb-20">
          ...
          <Section title="Content">
            <Field label="Title"><input type="text" value={form.title} onChange={(e) => { update("title", e.target.value); if (!editId) update("slug", slugify(e.target.value)); }} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
            <Field label="Slug"><input type="text" value={form.slug} onChange={(e) => update("slug", slugify(e.target.value))} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none font-mono text-xs" /></Field>
            <Field label="Short Description"><textarea rows={3} value={form.shortDescription} onChange={(e) => update("shortDescription", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
            <Field label="Full Article">
              <Suspense fallback={<div className="h-[500px] rounded-2xl border border-zinc-200 dark:border-white/[0.08] bg-zinc-50 dark:bg-[#0F2040] flex items-center justify-center text-sm text-zinc-400">Loading editor...</div>}>
          ...
          <Section title="Taxonomy">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Category"><select value={form.category} onChange={(e) => update("category", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none">{CATEGORIES.map((c) => <option key={c}>{c}</option>)}</select></Field>
              <Field label="Author"><input type="text" value={form.author} onChange={(e) => update("author", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
            </div>
            <Field label="Tags (comma separated)"><input type="text" value={form.tags} onChange={(e) => update("tags", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" placeholder="web development, react, tailwind" /></Field>
          </Section>
          ...
          <Section title="Images">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Image Alt Text"><input type="text" value={form.imageAlt} onChange={(e) => update("imageAlt", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
              <Field label="Image Title"><input type="text" value={form.imageTitle} onChange={(e) => update("imageTitle", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
              <Field label="Image Caption"><input type="text" value={form.imageCaption} onChange={(e) => update("imageCaption", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
              <Field label="Image Description"><input type="text" value={form.imageDescription} onChange={(e) => update("imageDescription", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
            </div>
          </Section>
          ...
          <Section title="Status & Publishing">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Field label="Status"><select value={form.status} onChange={(e) => update("status", e.target.value as Status)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none">
                <option value="draft">Draft</option><option value="published">Published</option><option value="scheduled">Scheduled</option>
              </select></Field>
              {form.status === "scheduled" && <Field label="Schedule Date"><input type="datetime-local" value={form.scheduledDate} onChange={(e) => update("scheduledDate", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>}
            </div>
          ...
          <Section title="SEO Settings">
            <Field label="SEO Title"><input type="text" value={form.seoTitle} onChange={(e) => update("seoTitle", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" placeholder="Leave blank to use post title" /></Field>
            <Field label="Meta Description"><textarea rows={2} value={form.metaDescription} onChange={(e) => update("metaDescription", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" placeholder="Brief description for search results" /></Field>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Focus Keyword"><input type="text" value={form.focusKeyword} onChange={(e) => update("focusKeyword", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
              <Field label="Canonical URL"><input type="text" value={form.canonicalUrl} onChange={(e) => update("canonicalUrl", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
            </div>
          ...
          <Section title="Social Sharing">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="OG Title"><input type="text" value={form.ogTitle} onChange={(e) => update("ogTitle", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
              <Field label="Twitter Card"><select value={form.twitterCard} onChange={(e) => update("twitterCard", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none">
            ...
            <Field label="OG Description"><textarea rows={2} value={form.ogDescription} onChange={(e) => update("ogDescription", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
          ...
          <Section title="Schema.org">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Schema Type"><select value={form.schemaType} onChange={(e) => update("schemaType", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none">
              ...
              <Field label="Breadcrumb Title"><input type="text" value={form.breadcrumbTitle} onChange={(e) => update("breadcrumbTitle", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" /></Field>
            </div>
          </Section>
            ...
            {form.faq.map((item, i) => (
              <div key={i} className="flex gap-3 items-start mb-3">
                <div className="flex-1 space-y-2">
                  <input type="text" placeholder="Question" value={item.question} onChange={(e) => updateFaq(i, "question", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" />
                  <textarea rows={2} placeholder="Answer" value={item.answer} onChange={(e) => updateFaq(i, "answer", e.target.value)} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none" />
                </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            ...
            <input type="text" placeholder="Search posts..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 pr-4 py-2 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#0F2040] text-sm focus:outline-none w-48" />
          </div>
          {(["all", "published", "draft", "scheduled"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-xl text-xs font-bold capitalize transition-colors ${filter === f ? "bg-blue-600 text-white" : "bg-zinc-100 dark:bg-[#14233F] text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700"}`}>{f}</button>
          ))}
        </div>
