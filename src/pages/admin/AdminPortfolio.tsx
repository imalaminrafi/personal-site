    ...
    <AdminLayout title="Portfolio">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            ...
            <input type="text" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 pr-4 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-white dark:bg-[#162032] text-sm focus:outline-none w-48" />
          </div>
          <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-white dark:bg-[#162032] text-sm focus:outline-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {filtered.map((item, index) => (
          <div key={item.id} className="bg-white dark:bg-[#162032] rounded-2xl border border-zinc-200 dark:border-[#1E3A5F] overflow-hidden group">
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-zinc-100 dark:border-[#1E3A5F]">
      ...
      {showModal && form && (
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white dark:bg-[#162032] rounded-3xl border border-zinc-200 dark:border-[#1E3A5F] shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-5 overflow-y-auto">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={form.title} onChange={(e) => update({ title: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input type="text" value={form.slug} placeholder={slugify(form.title) || "auto-generated"} onChange={(e) => update({ slug: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                    </div>
                  </div>
                </div>
                <div className="mt-4">
                  <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">Description</label>
                  <textarea rows={3} value={form.description} onChange={(e) => update({ description: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                  <div>
                    <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">Category</label>
                    <select value={form.category} onChange={(e) => update({ category: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-white dark:bg-[#162032] text-sm focus:outline-none">
                  ...
                  <div>
                    ...
                    <input type="text" value={form.tags.join(", ")} onChange={(e) => update({ tags: e.target.value.split(",").map((t) => t.trim()).filter(Boolean) })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                  </div>
                </div>
                <div className="mt-4">
                  ...
                  <div className="flex items-center gap-2">
                    <input type="url" value={form.liveUrl} placeholder="https://your-project.vercel.app" onChange={(e) => update({ liveUrl: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                  </div>
                ...
                <label className="flex items-center gap-2 mt-4 text-xs font-bold text-zinc-600 dark:text-zinc-300 cursor-pointer">
                  <input type="checkbox" checked={form.featured} onChange={(e) => update({ featured: e.target.checked })} className="rounded border-zinc-300 dark:border-[#1E3A5F]" />
                  Featured project
                </label>
              </div>
              ...
              <div>
                ...
                <div>
                  ...
                  <textarea rows={2} value={form.caseStudy.challenge} onChange={(e) => update({ caseStudy: { ...form.caseStudy, challenge: e.target.value } })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
                <div className="mt-3">
                  <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">The Approach</label>
                  <textarea rows={2} value={form.caseStudy.approach} onChange={(e) => update({ caseStudy: { ...form.caseStudy, approach: e.target.value } })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
                <div className="mt-3">
                  <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">Results (one per line)</label>
                  <textarea rows={3} value={resultsText} onChange={(e) => setResultsText(e.target.value)} placeholder={"3x faster page loads\nCheckout completion up by 38%"} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={form.seoTitle || ""} onChange={(e) => update({ seoTitle: e.target.value })} placeholder={form.title} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">SEO Keywords</label>
                    <input type="text" value={form.seoKeywords || ""} onChange={(e) => update({ seoKeywords: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                  </div>
                </div>
                <div className="mt-4">
                  <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">Meta Description</label>
                  <textarea rows={2} value={form.seoDescription || ""} onChange={(e) => update({ seoDescription: e.target.value })} placeholder={form.description} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
              </div>
            </div>

            <div className="flex justify-end gap-3 p-6 pt-0 shrink-0 border-t border-zinc-100 dark:border-[#1E3A5F]">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={reviewEditor.clientName} onChange={(e) => setReviewEditor({ ...reviewEditor, clientName: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">Platform (Upwork, Fiverr...)</label>
                  <input type="text" value={reviewEditor.platform} onChange={(e) => setReviewEditor({ ...reviewEditor, platform: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">Country (optional)</label>
                  <input type="text" value={reviewEditor.country || ""} onChange={(e) => setReviewEditor({ ...reviewEditor, country: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">Project Date</label>
                  <input type="date" value={reviewEditor.projectDate} onChange={(e) => setReviewEditor({ ...reviewEditor, projectDate: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-300 mb-1">Review Text *</label>
                <textarea rows={4} value={reviewEditor.text} onChange={(e) => setReviewEditor({ ...reviewEditor, text: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-zinc-200 dark:border-[#1E3A5F] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
              </div>
