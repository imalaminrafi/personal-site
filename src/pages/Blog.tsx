      ...
      <main>
        <section className="relative pt-32 pb-20 px-6 overflow-hidden">
          ...
          <div className="max-w-6xl mx-auto relative">
            <p className="text-xs font-semibold uppercase tracking-widest text-violet-600 dark:text-violet-400 mb-3">Insights</p>
            <h1 className="text-4xl sm:text-5xl font-black text-zinc-900 dark:text-white mb-4">Growth Insights</h1>
            <p className="text-zinc-500 dark:text-zinc-400 max-w-xl text-sm">Quick reads on AI, business &amp; digital strategy — practical guides, tips, and stories about building a business online.</p>
          </div>
        </section>
        ...
        <section className="max-w-6xl mx-auto px-6 pb-8">
          <div className="flex flex-wrap items-center gap-3 mb-8">
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              ...
              <input type="text" placeholder="Search articles..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#162032] text-sm focus:outline-none focus:border-violet-400 dark:focus:border-violet-500 transition-colors" />
            </div>
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#162032] text-sm focus:outline-none">
              {["All", ...CATEGORIES].map((c) => <option key={c}>{c}</option>)}
            </select>
            <select value={sort} onChange={(e) => setSort(e.target.value as "newest" | "oldest")} className="px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#162032] text-sm focus:outline-none">
          ...
          {paginated.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {paginated.map((post) => (
                  <Link key={post.id} to={`/blog/${post.slug}`} className="group flex flex-col rounded-2xl overflow-hidden border border-zinc-100 dark:border-white/[0.05] bg-zinc-50/50 dark:bg-[#162032] hover:border-violet-200 dark:hover:border-violet-800/40 hover:shadow-lg hover:shadow-violet-500/5 transition-all duration-300">
