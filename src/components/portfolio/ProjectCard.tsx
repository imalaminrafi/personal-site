...
export default function ProjectCard({ project }: { project: PortfolioItem }) {
  ...
  return (
    <article className="group bg-white dark:bg-[#0d0b1f] rounded-2xl overflow-hidden border border-zinc-200 dark:border-white/[0.08] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
      {/* Featured image */}
      <Link to={`/portfolio/${project.slug}`} className="relative aspect-[16/10] overflow-hidden block bg-zinc-100 dark:bg-zinc-900">
        {project.image ? (
          <img
