...
export default function GrowthInsights() {
    ...
    return (
        <section id="insights" className="bg-white dark:bg-[#0F172A] py-12 sm:py-14 border-t border-zinc-100 dark:border-white/[0.05]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5">
                    {posts.map((post) => (
                        <Link
                            key={post.id}
                            to={`/blog/${post.slug}`}
                            className="group flex flex-col rounded-xl overflow-hidden border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#162032] hover:border-[#C9A84C]/50 dark:hover:border-[#C9A84C]/40 hover:shadow-lg transition-all duration-300"
                        >
