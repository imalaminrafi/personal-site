...
export default function WysiwygEditor({ value, onChange, onAutoSave }: WysiwygEditorProps) {
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 dark:border-[#1E3A5F] overflow-hidden bg-white dark:bg-[#162032]">
      <div className="flex flex-wrap items-center gap-0.5 px-3 py-2 border-b border-zinc-200 dark:border-[#1E3A5F] bg-zinc-50/50 dark:bg-[#162032]/50 sticky top-0 z-10">
        {/* History */}
        ...
        <div className="relative">
          ...
          {showFontSize && (
            <div className="absolute top-full left-0 mt-1 bg-white dark:bg-[#1E293B] rounded-xl border border-zinc-200 dark:border-[#1E3A5F] shadow-xl p-1.5 flex gap-1 z-20">
        ...
        <div className="relative">
          ...
          {showColors && (
            <div className="absolute top-full left-0 mt-1 bg-white dark:bg-[#1E293B] rounded-xl border border-zinc-200 dark:border-[#1E3A5F] shadow-xl p-2 z-20">
              <div className="grid grid-cols-8 gap-1">
                {["#000000","#ffffff","#dc2626","#ea580c","#d97706","#65a30d","#16a34a","#0891b2","#2563eb","#C9A84C","#db2777","#78716c"].map((c) => (
                  <button key={c} type="button" onClick={() => setColor(c)} className="w-6 h-6 rounded-lg border border-zinc-200 dark:border-[#1E3A5F] hover:scale-110 transition-transform" style={{ backgroundColor: c }} title={c} />
                ))}
              </div>
        ...
        {showLinkInput && (
          <div className="flex items-center gap-2 ml-2">
            <input type="url" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} placeholder="https://..." className="px-2 py-1 rounded-lg border border-zinc-200 dark:border-[#1E3A5F] bg-white dark:bg-[#162032] text-xs w-48 focus:outline-none" autoFocus onKeyDown={(e) => { if (e.key === "Enter") addLink(); if (e.key === "Escape") setShowLinkInput(false); }} />
      ...
      <div className="flex items-center justify-between px-6 py-2 border-t border-zinc-200 dark:border-[#1E3A5F] bg-zinc-50/50 dark:bg-[#162032]/50 text-[11px] text-zinc-500">
